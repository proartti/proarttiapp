...
8. Configure DB connection strategy for DreamHost MySQL and create migration strategy (idempotent forward-only migrations with seeded baseline data for plans/statuses).
9. Configure frontend build/deploy workflow for shared hosting (build assets in CI, publish compiled assets only to runtime path).
10. Phase 3 - Identity and Access (parallel with Phase 4 after core bootstrap)
10. Implement passwordless login via signed email magic links with expiration, one-time usage, and audit trail.
11. Integrate Google OAuth for social sign-in with account-linking rules (same email merges to one user identity).
12. Define authorization model (admin/operator roles) even if minimal in MVP so API and web permissions stay consistent.
13. Phase 4 - Core Domain MVP (parallel with Phase 3)
14. Model domain entities for MVP: Client + related contact and lifecycle fields; keep schema extensible for future subscription records.
15. Implement Client CRUD in web UI (Inertia pages, server-side validation, pagination/filtering, optimistic UX where safe).
16. Implement matching API endpoints for Client CRUD in /api/v1 with resource transformers, validation requests, and consistent error envelope.
17. Add basic activity history for client changes (created/updated/archived) to support traceability and mobile consistency later.
18. Phase 5 - API-First Expansion Foundation (depends on Phases 3-4)
19. Publish API contract artifacts (OpenAPI or equivalent) and enforce response conventions (pagination/meta/errors).
20. Introduce token strategy for mobile/API consumers (Sanctum personal access tokens or OAuth flow) while keeping browser auth session-based.
21. Add rate limiting and API observability baseline (request IDs, structured logs, failure metrics).
22. Phase 6 - GitHub Actions Deployment and DreamHost Operations (depends on all prior phases)
23. Implement CI workflow on GitHub Actions for lint/test/build on every pull request and push to develop and main.
24. Configure two GitHub Actions Environments: staging and production, each with isolated secrets, environment URL, and protection rules.
25. Implement staging CD workflow: auto-deploy on push to develop using staging secrets and staging host/path.
26. Implement production CD workflow: deploy from main only after required approval gate in production environment.
27. Define non-interactive release script shared by both environments: maintenance on, sync release, install dependencies, migrate --force, optimize/cache warmup, maintenance off, health check, rollback path.
28. Configure production-safe queue and scheduler approach compatible with DreamHost constraints (cron-driven scheduler, async jobs where feasible).
29. Add fallback deploy option if hosted runner cannot reach DreamHost (manual SSH deploy script or DreamHost-triggered git pull).
30. Set backups and recovery checklist for DB and uploaded assets.
31. Phase 7 - Quality Gates and Go-Live Readiness
27. Add testing pyramid baseline: unit tests for domain rules, feature tests for auth and Client CRUD (web + API), and smoke tests for deploy validation.
28. Execute UAT checklist (auth flows, CRUD correctness, API parity, error handling, pagination, permission checks).
29. Prepare post-MVP roadmap to add subscriptions module (plans, contracts, renewals/cancellations, billing integration) without breaking current API contracts.
